...
class RoutesWindow(QWidget):
    ...
    def load_combos(self):
        ...
        try:
            ...
            cursor.execute("""SELECT e.id_employee, e.last_name, e.first_name, p.appointment
                              ...
                              WHERE p.appointment = 'Диспетчер' OR p.appointment IS NULL
                              ORDER BY e.id_employee""")
            employees = cursor.fetchall()
            self.employee_combo.clear()
            for emp in employees:
                position = emp[3] if emp[3] else "Диспетчер"
                self.employee_combo.addItem(f"{emp[1]} {emp[2]} ({position})", emp[0])
    ...
    def delete_route(self):
        ...
        reply = QMessageBox.question(self, "Подтверждение", "Удалить этот маршрут?",
        ...
        try:
            ...
            conn.commit()
            QMessageBox.information(self, "Успех", "Маршрут удален!")
            self.clear_inputs()
            self.load_data()
        except Exception as e:
    ...
    def apply_role_restrictions(self):
        if self.role == "employee":
            ...
            self.departure_date_input.setEnabled(False)
            self.arrival_date_input.setEnabled(False)
            self.destination_input.setReadOnly(True)
            self.fuel_input.setEnabled(False)
